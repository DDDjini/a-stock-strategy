...
trades_df.to_csv("results/trades_optimized_expanded.csv", index=False)
equity_df = pd.DataFrame(equity_curve)
equity_df.to_csv("results/equity_optimized_expanded.csv", index=False)

print(f"\n{'='*60}")
print(f"回测结果（{len(stock_data)}只股票，近3年）")
print(f"{'='*60}")
...
print(f"\n已保存 results/trades_optimized_expanded.csv 和 results/equity_optimized_expanded.csv")
